...
Page({
  ...
  addGoodsget() { //添加购物车
    ...
    let config = {
      'goods_id': activeGood,
      'goods_num': active_good_num,
      'item_key': active_spec_data.join("_"),
      garnish: appendList.join()
    };
  ...
  toSum() { //结算 ----重点 调用支付宝刷脸付逻辑
    if (!this.data.cardSumMoney) return wx.showToast({
      icon: "none",
      title: '购物车为空'
    });
    wx.showToast({
